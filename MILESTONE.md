...
- [ ] SearchBackend を stringly-typed から tagged enum に移行（Rust: serde tagged enum, TS: discriminated union）
- [ ] WSL コマンド実行の共通化（search/git/fs の WSL dispatch を `ShellConfig::command()` に統合）
