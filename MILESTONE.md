...
- [ ] pike CLI（`pike` コマンドで外部ターミナルからプロジェクト/ファイルを開く、ターミナル内から `pike open <file>` でエディタタブを開く。VS Code の `code` CLI に相当）
...
- [ ] リポジトリ公開（MIT LICENSE 追加、Cargo.toml/package.json のライセンス・author 統一、.gitignore 安全ネット追加、GitHub で public に切替）
