...
| M5.5 | ファイル操作拡張 | プレビュー・文字コード・D&D | ⬜ 未着手 |
...
**プレビュー**
- [ ] 画像ファイル（png/jpg/gif/svg/webp）を開いたらプレビュータブを表示
- [ ] Markdown ファイルのプレビュー表示（エディタとの切替 or 分割）

**エディタ情報表示**
- [ ] フッターにカーソル位置（行:列）、文字コード、改行コード、ファイルタイプを表示
- [ ] 文字コード指定での開き直し・保存をサポート
- [ ] 改行コードの変更をサポート（LF/CRLF）

**ファイルツリー操作**
- [ ] コンテキストメニュー: 名前の変更、ファイル削除
- [ ] コンテキストメニュー: Git History 表示
- [ ] ドラッグ&ドロップによるファイル/フォルダ移動
- [ ] Ctrl+ドラッグでコピー

**Git パネル連携**
- [ ] COMMITS のファイルにコンテキストメニュー追加（「変更を開く」「ファイルを開く」）
...
- [ ] rg のサイドカーバンドル検討
- [ ] アイコンライブラリ導入（Lucide 等）とアプリ全体のアイコン見直し（サイドバー・タブ・ファイルツリー・Git ステータス等）
