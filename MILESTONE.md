...
- [ ] `docker exec` でコンテナ内シェルに入る機能（ターミナルタブで開く）
- [ ] エディタ ミニマップ（CodeMirror 6 拡張 or カスタム描画）
- [ ] エディタ内検索・置換（Ctrl+F / Ctrl+H、マッチハイライト）
- [ ] エディタ git diff 変更行強調（追加/変更行をガター+スクロールバーに表示）
- [ ] テーマの概念導入と設定による切り替え（ダーク/ライト等）
- [ ] i18n 対応（UI テキストの多言語化）
- [ ] Git が参照する git プロセスの切り替え対応（1Password SSH Agent 等、カスタム git 実行パスの指定）
