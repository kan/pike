...
| M8 | セッション永続化 | tmux 復元・プロジェクト状態 | ⬜ 未着手 |
...
### 目的
「Windows Tauri → WSL2 → tmux → Claude Code」の多層構造が動くことを最小コードで確認する。
...
- [ ] ウィンドウリサイズで xterm.js と PTY のサイズが追従する

**Step 1-B: tmux 経由接続**
- [ ] `wsl.exe tmux new-session -s test` で tmux セッションを起動できる
- [ ] セッションが既存なら `tmux attach-session -t test` でアタッチできる
- [ ] アプリを閉じて再起動しても tmux セッションが生きていれば再接続できる

**Step 1-C: Claude Code の TUI 描画**
- [ ] tmux セッション内で `claude` コマンドを起動できる
- [ ] Claude Code の TUI（色・Unicode・カーソル移動）が崩れない
- [ ] ターミナルリサイズ後も表示が正しく再描画される
- [ ] Ctrl+C 等の特殊キーが正しく伝達される

**Step 1-D: Docker socket 接続（別途 cargo run で検証）**
- [ ] Windows 側 Rust から bollard で `//./pipe/docker_engine` に接続できる
- [ ] `docker ps` 相当（コンテナ一覧取得）が動く
- [ ] コンテナのログストリームを取得できる
...
### 完了条件
- [ ] tmux セッションの生存確認（🟢/⚪ 表示）がプロジェクト一覧に出る
- [ ] 固定タブのプロセスが生きていれば attach、死んでいれば auto_start コマンドで再起動
- [ ] タブの並び順・アクティブタブを project.json に保存・復元する
- [ ] アプリ終了時に非固定タブのセッションは破棄、固定タブは tmux に残す
