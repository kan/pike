...
- **AI エージェント**：Claude Code / Codex / Copilot CLI / opencode をターミナルで直接動かす前提の導線。インストール済みのものだけを出す起動ボタン、過去セッションの再開、定型プロンプトの挿入、エージェントが入力を待ったときの知らせ
- **エージェント状態**：アカウント・利用率・トークン使用量とコストを、使っているエージェントごとに確認。複数アカウント（`CLAUDE_CONFIG_DIR`）にも追従し、Claude Code の hook から実際のアカウントを受け取れる
- **ターミナルの AI 補助**：出力中のパスをクリックしてファイル・画像・ディレクトリを開く（行番号の有無を問わない）、エディタの選択範囲や行の参照、診断、issue の着手指示をターミナルへ送る
...
- **プレビュー**：Markdown / reStructuredText / Mermaid / CSV / JSON / SVG / PDF / HTML / Vue（Vite の開発サーバーで描画）、外部画像はドメイン単位で許可、表示専用の画像ビューア
