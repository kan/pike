...
このリポジトリをマーケットプレイスとして登録し、プラグインを入れる。

```bash
# GitHub からマーケットプレイスを登録
claude plugin marketplace add kan/pike
# プラグインを導入
claude plugin install pike-todo@pike
```

ローカルのクローンから入れたい場合は、リポジトリ直下（`plugins/` ではなく
`.claude-plugin/marketplace.json` があるルート）をパスで指定する。

```bash
claude plugin marketplace add /path/to/pike
claude plugin install pike-todo@pike
```
