...
- **出力寸法はサイズクラスごとに固定**：`prepare()` が `setWindowSize()` で外形を固定するため、
  ...
  - **全体レイアウト（`layout.ts` の `FULL`）**：外形 1600×1000 → 内寸 **1578×945**。
    screen-layout / global-* など、実使用時の画面比率を見せる撮影。`panels.ts` の
    `COMMIT_SIZE`（commit-tab）も同寸で、被写体が 3 分割のタブなのでこちら側に入れてある
  - **外枠付きヒーロー（`layout.ts` の `HERO` = `FULL` の 2 倍）**：外形 3200×2000 →
    内寸 **3179×1944**。overview / hero-editor / hero-git。掲載時は幅に合わせて縮小される
    ので高精細な素材になり、split プレビューのような縦に長い被写体も見切れない
    - プライマリモニタ（3413×1440）より縦が大きいが、WebView2 の撮影は画面外にはみ出した
      分も含めて撮れる。逆に DPR を上げる方向（`--force-device-scale-factor=2`）は撮影が
...
- タイトルバー（アイコン + "Pike" + 最小化/最大化/閉じるのキャプションボタン）を上に足し、
  四隅を角丸にマスクし、ヘアラインとドロップシャドウを重ねる。テーマはファイル名の
  `-dark` / `-light` サフィックスから判定（`THEME=` で明示指定も可）。
- **枠の寸法は素材の幅から倍率を決める**（`s = round(w / 1600)`）。タイトルバー高・角丸半径・
  キャプションボタン幅・アイコン・文字サイズ・影・余白のすべてに掛かるので、`HERO`（2 倍）の
