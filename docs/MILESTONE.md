...
| M14 | Codex IDE 統合強化 | スラッシュコマンド・通知・コンテキスト注入・Sandbox設定 | ✅ 完了 |
...
**Diff 連携**（Codex 側の diff プロトコル未対応のため見送り）
...
**スラッシュコマンド**
- [x] `/compact` — `thread/compact/start` で会話コンテキストを圧縮
- [x] `/clear` — チャット履歴クリア + 新規スレッド開始
- [x] `/model` — `model/list` でモデル一覧取得 + `turn/start` の `model` パラメータで切替
- [x] `/read <path>` — 指定ファイルの内容をコンテキストに注入（`input` に `type: "text"` で追加）
- [x] `/diff` — 現在の作業ツリーの git diff を取得してコンテキストに注入
- [x] コマンドパレット風 UI（`/` 入力でドロップダウン補完）

**@ メンション補完**
- [x] チャット入力欄で `@` を入力するとプロジェクト内ファイルパスの補完候補を表示
- [x] `rg --files` の結果をキャッシュし、ファジーマッチで絞り込み
- [x] 選択されたファイルの内容を turn の `input` にコンテキストとして追加
- [x] `@` + ディレクトリパスでディレクトリ内のファイル一覧も注入可能に
...
- [x] Codex タブの UI に「AGENTS.md detected」/「CLAUDE.md detected」インジケータ表示

**Sandbox 設定 UI**
- [x] Settings タブに Codex セクション追加
- [x] sandbox モード選択（workspace-write / danger-full-access / externalSandbox）
- [x] approval ポリシー選択（untrusted / on-failure / on-request / never）
- [x] Windows で workspace-write 選択時は警告を表示（クラッシュリスクの説明）

**チャット UX 改善**
- [x] コマンド実行結果の stdout/stderr をチャット内に折りたたみ表示
- [x] FileChange アイテムに変更ファイルパスと変更行数のサマリーを表示
- [x] エージェントの reasoning/thinking サマリーを折りたたみ表示
- [x] チャット履歴の検索機能
- [x] ターン単位のロールバック（`thread/rollback`）

**その他**
- [x] Codex プロセス異常終了時の自動再接続とエラー表示
- [x] トークン使用量の表示（`thread/tokenUsage/updated` から取得）
- [ ] ~~複数スレッド管理（`thread/list` で一覧取得、切替 UI）~~ — 見送り
