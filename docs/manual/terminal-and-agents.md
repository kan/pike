...
- **複数アカウントの使い分け**：Claude Code の `CLAUDE_CONFIG_DIR` をシェルの設定ファイルか `.envrc` に書いていれば、Pike もその設定ディレクトリを見て集計します。Claude Code チャットタブも同じアカウントで起動します。内訳ドロップダウンの先頭にログイン中のアカウント（メールアドレス、組織、プラン）が出るので、どのアカウントの数字かを確認できます。既定の場所から動かしているときは、その設定ディレクトリも並べて表示します。Windows のシェルでは Pike に引き継がれた環境変数を見るため、PowerShell のプロファイルの中だけで設定した値は反映されません。
