...
設定の [閉じたときトレイに最小化](settings.md#全般) を切っている場合、メインウィンドウの `✕` は Pike 自体の終了になります。このときは他のウィンドウを含めた実行中の件数を数えて確認します。トレイに最小化する設定のままなら、ウィンドウが隠れるだけでプロセスも動き続けるため、確認は出ません。
