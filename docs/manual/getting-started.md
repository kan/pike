...
インストール後は GitHub Releases を参照して自動でアップデートを確認します。手動で確認したい場合は、左下の歯車メニュー → 「更新を確認」、または設定タブの「バージョン情報」セクションから行えます。
...
- **エクスプローラーからドロップ**：ファイルをタブバーへドラッグ&ドロップすると、対応するタブ（エディタ・画像・PDF）で開きます。フォルダをドロップすると、そのフォルダを作業ディレクトリにしたターミナルタブが開きます（Windows プロジェクトとグローバルモードのみ）
