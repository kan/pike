...
NSIS インストーラは、インストール先をユーザーの PATH に登録して `pike` コマンドをターミナルから使えるようにし、エクスプローラーの「プログラムから開く」候補に Pike を追加します（既定の関連付けは変更しません。どちらもアンインストールで削除されます）。MSI インストーラにはこの登録がありません。

インストール後は GitHub Releases を参照して**自動でアップデートを確認**します。手動で確認したい場合は、左下の歯車メニュー → 「更新を確認」、または設定タブの「バージョン情報」セクションから行えます。
