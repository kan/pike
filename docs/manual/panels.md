...
**問題** パネルは、プロジェクトで使っているツールチェインのチェッカーを実行して、エラーと警告を一覧表示します。常駐する言語サーバーは持たず、パネルを開いたときに 1 回だけ走らせる方式です（Pike が軽い理由のひとつ）。
...
Go モジュールが golangci-lint を使う設定になっていると、パネル上部に **golangci-lint** ボタンが出ます。目印は次のどちらかです。

- 設定ファイル（`.golangci.yml` / `.golangci.yaml` / `.golangci.toml` / `.golangci.json`）がモジュールと同じ階層か、その上にある
- `go.mod` が golangci-lint を参照している
