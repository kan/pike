...
- **`.gitignore` のファイル/ディレクトリ**はグレーの淡色で示します。ディレクトリは展開できます。`node_modules` などの除外ディレクトリ（`IGNORED_DIRS`）も淡色ですが、こちらは展開しません。
...
- **compose up / down**：パネル上部の再生 / 停止ボタンで `docker compose up -d` / `docker compose down` を実行できます。確認ダイアログ付きで、出力はターミナルタブに表示されます。
