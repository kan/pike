...
Pike は「AI エージェント × ターミナル」に特化した、**軽量な開発環境**です。VS Code より軽いことを差別化点に置いています。主な対象は Windows で、macOS ではローカルのシェルを使って開発できるところまで対応します。エディタとターミナル、Docker のログを同じタブバーで扱い、Git や検索、ファイルツリーは左サイドバーのパネルにまとめています。
