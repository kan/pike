...
Pike は「AI エージェント × ターミナル」に特化した、**軽量な Windows 向け開発環境**です。VS Code より軽いことを差別化点に置いています。エディタとターミナル、エージェントのチャット、Docker のログを同じタブバーで扱い、Git や検索、ファイルツリーは左サイドバーのパネルにまとめています。
