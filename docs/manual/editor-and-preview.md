...
- **`Ctrl+H`**：置換（検索パネルを置換行付きで開きます。パネル左端の折り畳みボタンからも開けます）。
...
- **マージコンフリクト**：`<<<<<<<` / `|||||||` / `=======` / `>>>>>>>` のマーカー行と各セクション本文を色分けハイライトします。あわせて、各コンフリクトの上に採用する側を選ぶボタンが並び、エディタ上部にファイル全体の一括適用が出ます。詳しくは [Git のエディタ上で解消する](git.md#エディタ上で解消する) を参照してください。
