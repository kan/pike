...
- **グローバルモードの既定シェル**：グローバルモードのウィンドウで「+」を押した時に開くシェル。3 種の Windows シェルと検出済みの WSL ディストロから選べます。マシンの WSL 構成に依存するため、この設定だけは設定同期の対象外です。→ [グローバルモード](global-mode.md#ターミナルとして使う)
...
- プロジェクト一覧やグループ、同期ファイルのパス自体、グローバルモードの既定シェルは同期対象外です（環境固有のため）。
