...
>
> `Ctrl+Shift+P`（プロジェクト切替）と `Ctrl+,`（設定）はシェルが使わないキーなので、ターミナル上でもそのまま使えます。クイックオープン（`Ctrl+P`）には代わりの操作が無いため、ターミナル以外のタブへ移ってから使ってください。
...
Windows のタスクバーとシステムトレイから、Pike の起動・プロジェクト切り替え・復帰を行えます（Windows 専用）。
...
- **アイコンにカーソルを合わせる**：Claude の 5 時間枠の使用率など、AI の使用量の概要をツールチップで表示します。
