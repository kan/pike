export default {
  ...
  'settings.sync': '設定の同期',
  'settings.syncHint':
    '設定・プロジェクト一覧とグループ・ブックマークを 1 つの JSON ファイルで複数の PC と共有します。Dropbox など同期フォルダの中のパスを指定し、「今すぐ同期」を押したときに同期します。両方の PC で同じ項目を別の値に変えていたら、どちらを採るかを選べます。',
  ...
  'sync.restoreNoTarget': '同期ファイルのパスが無いため、戻せません。',
