...
fn hook_command(shell: &ShellConfig) -> String {
    ...
    //
    // **通知（#265）も共有する**（#333）。`wait::send_notice_to_first_instance` が
    // 走っている Pike を両方の identifier で探すので、どちらのビルドが書いた行でも、
    // そのとき走っているほうへ届く。**ここを非対称にしないこと**: 通知だけがビルド
    // 固有だったころは、緩い一致（`matches_spec`）と噛み合って、インストール版の
    // 設定画面が開発版の行を「登録済み」と出し、解除すると相手の行を消していた。
    //
    // **共有されるのは届け先であって、書いた exe ではない。** 開発ビルドの行が指すのは
    // `target/debug` なので、そのビルドを消せば hook 自体が走らない（`remove_hook`）。
    // 緩い一致は「登録済み」と出したままなので、消したときは登録し直す。
...
/// 足した hook を取り除く。**消えるのは Pike が足した行のどれも**（`remove_hook` の
/// 緩い一致。開発ビルドが書いた絶対パスの行も含む）で、利用者が置いた別の hook は残る。
///
