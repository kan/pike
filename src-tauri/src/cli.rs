...
pub enum CliAction {
    ...
    /// 走っているウィンドウで、このプロジェクト（登録済み / 一時）へ切り替える（#352）。
    /// `pike <dir>` を **Pike のターミナルから**叩いたときに、そのウィンドウへ届く。
    ///
    /// **上の 2 つと分けてある。** `OpenProject` は昇格再起動の経路（セッションを復元せず
    /// ターミナルを 1 枚足す）、`FocusProject` はコールドスタート専用（前回のセッションを
    /// 復元してから行く）で、どちらも既に走っているウィンドウの切り替えではない。
    /// フロントの受け口は `adoptProject`＝「ウィンドウがプロジェクトを渡された」経路で、
    /// 一時プロジェクトの解決と登録の提案（#230）もそこが持っている。
    AdoptProject {
        id: String,
    },
    None,
