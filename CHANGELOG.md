...
- **Diff タブに検索を追加**: エディタと同じ `Ctrl+F` で検索パネルを開き、一致箇所へジャンプできます。大文字小文字の区別トグル、件数表示、`Enter` / `Shift+Enter` での前後移動、`Esc` で閉じる、現在の一致を中央にスクロールします。左右どちらのペインの一致も探し、差分の文字ハイライトと重ねて表示します (#176)
- **アウトラインのクリックでプレビューもスクロールするようにした**: Markdown をプレビュー（またはスプリット）で見ているとき、アウトラインの見出しをクリックするとプレビュー側もその見出しへスクロールします。見出しはスラッグで正確に位置合わせし、見出し以外は行の位置に合わせておおよそのところへ動きます (#177)
- **パネルで既に別ウィンドウで開いているプロジェクトはフォーカスするようにした**: プロジェクトパネルで選んだプロジェクトが別のウィンドウで開いている場合、現在のウィンドウを切り替えず、そのウィンドウへフォーカスします。どこにも開いていなければ従来どおり現在のウィンドウで切り替えます

### Bug Fixes

- **別ウィンドウでプロジェクトを開くときの誤動作を修正**: プロジェクトを切り替えたウィンドウから同じプロジェクトを別ウィンドウで開こうとしても何も起きなかったり、別のウィンドウにフォーカスされたりしていました。ウィンドウの識別子（ラベル）が生成時のプロジェクトに固定されたままで、切り替え後の実際のプロジェクトとずれるのが原因です。ラベルを識別専用にし、どのプロジェクトを表示しているかは内部のマップで一元管理するようにしました。あわせて、ウィンドウを閉じたときに間違ったプロジェクトが開いた一覧から消える・残る問題、ウィンドウ再読み込みで別のプロジェクトが復元される問題、コマンドライン経由で同じプロジェクトのウィンドウが二重に開く問題も解消しました (#175)

## [0.30.0] - 2026-07-20

### Features

- **手元に無いプロジェクトを clone できるようにした**: プロジェクトパネルを開くと、登録されているルートがこのマシンにあるかを確認します。無いものには「未取得」の印が付き、remote origin の URL を控えてあれば雲のアイコンから `git clone` を実行できます。clone はターミナルタブで走るので、パスフレーズや 2 要素認証のプロンプトにそのまま応答できます。終わると一覧を更新し、そのプロジェクトに切り替えるかを確認します (#164)
- **remote origin の URL を控えるようにした**: Git パネルが URL を読めたときと、プロジェクトパネルを開いたときの一括確認で、プロジェクト設定に保存します。以前から登録していたプロジェクトも、パネルを一度開けば入ります。URL のあるプロジェクトは一覧で名前の右に雲のアイコンが付きます (#164)
- **プロジェクト一覧を複数 PC で共有できるようにした**: 設定の同期ファイルに、名前・カラー・グループ・remote origin・base ディレクトリからの相対パスを書き出します。実際のパスは各 PC の base ディレクトリ（Windows 用・WSL 用を設定画面で指定）から解決するので、置き場所が違っても共有できます。別 PC で登録したリポジトリは「未取得」として並び、そのまま clone できます。書き込みも読み込みも空いている項目を埋めるだけで、他 PC の古い値が手元の編集を消すことはありません。削除したプロジェクトはその PC では非表示として記録し、次の読み込みで復活しません（設定画面から戻せます） (#164)
- **未 push / 未 pull をサイドバーに表示するようにした**: リモートより進んでいると Git アイコンの右下に「↑」、遅れていると「↓」が出ます。件数はアイコンのツールチップで確認できます
- **完了した TODO をまとめて消せるようにした**: TODO パネルのヘッダに「完了したものを削除」を追加しました。CLI では `pike todo clear --done` です

## [0.29.1] - 2026-07-20

### Bug Fixes

- **日本語入力で文字が消える不具合を修正**: ターミナルで変換した文字が、ときどき確定した文字ではなく後退（バックスペース）として送られ、入力した文字と直前の文字がまとめて消えることがありました。変換なしの入力を挟んだ直後に、SKK の `wo`→「を」のような短い変換をしたときに起きます。確定した文字を控えておき、直後に届いた後退をその文字へ差し替えます
- **`pike todo` がエージェントの画面を汚す不具合を修正**: Pike のターミナルで動かしているコーディングエージェントに `pike todo` を実行させると、CLI の出力や使い方の説明が画面に焼き付いて残っていました。CLI が親のコンソールへ接続する際に、呼び出し元へ渡す出力まで画面側へ向けてしまうのが原因です。出力の渡し先が決まっている場合は接続しないようにしました。端末から手で実行したときの表示は従来どおりです

## [0.29.0] - 2026-07-20

### Features

- **TODO にタイトルと詳細を持たせた**: タスクを 1 行のタイトルと、任意の複数行の本文で書けるようにしました。行頭の三角で詳細欄を開閉し、詳細を持つタスクは行の右端にアイコンが出るため、畳んだままでも見分けられます。ファイル上ではタスク行の下にインデントした行として保存するので、GitHub でもそのまま読めますし、これまでの 1 行だけの `todo.md` もそのまま使えます (#163)
- **`pike todo` に詳細の読み書きを追加**: `pike todo show <番号>` で本文込みの表示、`pike todo detail <番号> <本文>` で本文の設定ができます。`add` と `detail` は `-d/--detail`（繰り返しで複数行、`-` で標準入力から）、`-a/--append`、`--clear` を受け付けます。一覧は本文を畳んで `(+N lines)` とだけ表示し、`--json` には `detail` が入ります。コーディングエージェント向けのスキル（`plugins/`）も更新済みです (#163)

## [0.28.2] - 2026-07-20

### Bug Fixes

- **使用中に固まって強制終了される不具合を修正**: 0.27.0 で追加したタスクバーのジャンプリストを、UI スレッド上で組み立てていました。この処理は Windows のシェルに「最近使った項目」を解決させます。WSL プロジェクトに渡しているネットワークパスの解決で長く待たされることがあり、その間はアプリ全体が応答しなくなって Windows に終了させられていました。組み立てを専用のスレッドへ移し、UI を止めないようにしました (#160)
- **不透明設定でも背景透過の描画経路に乗り続けていた問題を修正**: 0.28.0 以降、背景の設定が既定の「不透明」でも、ウィンドウは透過合成のまま動いていました。不透明を選んだ場合は透過を解除し、従来の描画経路へ戻します (#162)

### Changes

- **ターミナルのデスクトップ通知を廃止**: プロセス終了時の通知と、ウィンドウが非アクティブなときにターミナルの出力を知らせる通知を取りやめました。設定「プロセス終了時にデスクトップ通知」も削除しています。タブのドット表示と終了コードのバッジは従来どおりです

## [0.28.1] - 2026-07-19

### Bug Fixes

- **背景透過中もポップアップを不透明に**: 背景透過（特にアクリル）を有効にすると、コンテキストメニュー・プルダウン・ダイアログ・ツールチップまで透けて読み辛くなっていました。これらの浮遊表示は透過の対象から外し、背景の透けで周囲が明るくなるぶんだけ色味を寄せて、ウィンドウから浮いて見えないようにしました (#162)

## [0.28.0] - 2026-07-19

### Features

- **背景透過（半透明・アクリル）**: ウィンドウ背景を半透明にして、背後のデスクトップを薄く見せながら作業できるようにしました。設定（外観 → 背景）で「不透明 / 透過（ブラー無し）/ アクリル」を切り替え、不透明度スライダー（10〜100%）で透け具合を調整できます。「透過」はデスクトップがくっきり透け、「アクリル」は Windows 11 のすりガラス効果です。サイドバー・パネル・ステータスバーに加え、ターミナルとエディタの本文も透過します（既定は不透明で従来どおりの見た目） (#162)

## [0.27.0] - 2026-07-18

### Features

- **Windows タスクバーのジャンプリスト**: タスクバーの Pike アイコンを右クリックしたときのメニューに、「新しいターミナルウィンドウ」と登録済みプロジェクト（最近開いた順）を追加しました。既定の「最近開いたファイル」も引き続き表示します (#160)
- **システムトレイに常駐**: Pike をシステムトレイに常駐させ、ウィンドウを閉じてもトレイに残して復帰できるようにしました。左クリックでメインウィンドウの表示/非表示を切り替え、右クリックメニューから新しいターミナル・最近のプロジェクト・プロジェクトスイッチャー・終了を実行できます。アイコンにカーソルを合わせると AI の使用量の概要をツールチップで表示します。「閉じたときトレイに最小化」の挙動は設定（全般）でオン/オフを切り替えられます（既定オン） (#161)
...
- **アプリ復帰後の IME 入力乱れを再修正**: v0.24.3 の「IME 無効化固着」対策の副作用で、他アプリから戻った直後の変換候補が非インラインになり、確定文字列が重複・順序異常になる問題が再発していました。無効化固着の対策は維持したまま、復帰完了後に入力コンテキストを再構築することで両方を解消しました

## [0.26.1] - 2026-07-16

### Security

- **Content Security Policy を設定**: これまで無効だった CSP を設定し、外部接続を IPC とマニュアル取得先のみに制限しました
- **Mermaid の描画をサニタイズ**: 図のラベル経由でスクリプトが実行されないよう、`securityLevel` を `antiscript` にしました（表示は従来どおり）
- **タスク名のコマンド注入を防止**: `package.json` の scripts と `deno.json` の tasks の名前に、シェルのメタ文字を含むものを実行対象から除外しました

### Bug Fixes

- **検索バックエンドの誤用を修正**: WSL プロジェクトと Windows プロジェクトを複数ウィンドウで併用したとき、Windows 用に同梱した rg を WSL 内で実行しようとして検索・タスク検出が失敗する場合がありました。シェルごとに検出結果を分けるようにしました

### Features

- **マニュアルをアプリのバージョンに合わせて表示**: これまで最新（main）のマニュアルを表示していましたが、インストール中のバージョンに対応するマニュアルを表示するようにしました。ツールバーに表示中のバージョンをバッジで示します（未リリース版などタグが無い場合は最新にフォールバック）
...
- **エクスプローラーの「プログラムから開く」に対応**: NSIS インストーラが Pike をアプリ候補として登録するようになりました。ファイルを右クリック → 「プログラムから開く」→「別のアプリを選択」で Pike を選べます（全ファイルタイプ対象。既定の関連付けは変更しません）。セルフアップデートでも登録されます
- **タブバーへのファイル / フォルダのドロップ**: エクスプローラーからタブバーへファイルをドラッグ&ドロップすると、対応するタブ（エディタ・画像・PDF）で開きます。フォルダをドロップすると、そのフォルダを作業ディレクトリにしたターミナルタブが開きます（Windows プロジェクトとグローバルモードのみ）

### Bug Fixes

- **未処理のファイルドロップで画面がファイル表示に置き換わる問題を防止**: ドロップ対象外の領域に OS のファイルを落とすと、WebView がファイルへ遷移して全ターミナルセッションが失われる可能性がありました。ウィンドウ全体でガードするようにしました

## [0.24.3] - 2026-07-16

### Bug Fixes

- **アプリ復帰時に IME が無効化されたまま固着する問題を修正**: v0.24.2 の緩和策（再フォーカス遅延の拡大）後も残っていた、他のアプリからターミナルへフォーカスを戻した際に IME が OFF のまま半角/全角キーも効かなくなる問題を修正しました。非アクティブ中のフォーカスの置き場を非編集要素から非表示のテキスト入力欄に変更し、復帰時に OS 側（TSF）へ IME 無効化状態が適用される状況そのものをなくしました
