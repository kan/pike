...
- **ログイン切れ（#381）**: `rate.rs` の `asks_for_login` が `/usage` の応答に `/login` の案内があるかを見て、`ClaudeRateLimits.login_required` → `AgentUsage.login_required` で運ぶ（未ログインは `Not logged in · Please run /login`、終了コード 1。空の `CLAUDE_CONFIG_DIR` で実測）。**検出のためにプロセスを増やさない**（`/usage` は元から定期的に走っている）。`claude auth status` は手元の認証情報を見るだけで、サーバー側で失効したトークンに気付けないので使わない
