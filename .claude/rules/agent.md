...
- **今のところ Rust に写しは無い。** `agents.rs` の `agent_detect` が受け取るのは「この
  名前のコマンドがあるか」だけ。ただし**この継ぎ目は起動までしか運べない**（使用量は
  ログの置き場と行の形式、フック駆動の通知は設定ディレクトリとフックの書式を Rust 側で
  知る必要がある）。そこまで来たら継ぎ目を `AgentId` に上げる、というのが表の doc の約束
- **検出できたものだけメニューに出す。** 押しても `command not found` になる項目を並べない。
  `wsl.exe` を余分に起こさないよう、**1 回のシェル起動で全部聞き**（`posix_probe_script` /
  `windows_probe_line`）、答えはシェルの導入単位でプロセスに 1 つ持つ（`IssuesState` と同じ形）
- **POSIX 側は対話ログインシェル（`-lic`）で聞く。** nvm / fnm / asdf / mise / Homebrew は
  ...
  同じ判断。`unset HISTFILE` とマーカー行（`PIKEAGENT`）が要る理由もあちらと同じ
- **TTL は 2 つあり、役割が違う。** Rust の `DETECT_TTL` は**プロセスを起こさない**ため、
...
- **ストアはシェルごとの表で持つ**（1 枠ではなく）。1 枠だと、同じウィンドウで WSL のタブと
  PowerShell のタブを行き来するだけで毎回聞き直し、`available` が「どのシェルの答えか」を
  知らないまま入れ替わる。表なら**プロジェクトを切り替えたときに捨てる必要も無い**
  （シェルが変われば別のキーを引くだけ）
- **メニューの並びは「既定のエージェント → 他のエージェント → 設定の行」。** 既定は
  `agentProfiles` の先頭（検出できたものの中で）で、ボタン本体もそれ。順序と既定を
  別々に持たないので、並べ替えたのに既定が変わらない、という食い違いが起きない。
  セッション一覧を出すかは「既定がそれ **or** 設定の行がそれを起動している」
  （`commandMentionsAgent`）で、wrapper 越しの起動を落とさない
...
- **種別ごとの機能をレジストリのフラグにしない。** セッション一覧（#220）は Claude 決め打ち
  （一覧・再開コマンド・見出しの全部）なので、`sessions?: true` のような欄を置くと
  「2 つ目に付けるとその名前の下に Claude の履歴が出る」型の通る嘘になる。読み手は
  `agentById('claude')` を直に見る
- **シェルの行に埋まる名前は Rust 側で検証する**（`is_safe_bin_name`）。表の値しか来ない
  前提でも、IPC の引数は誰でも投げられる
- **種別をコマンド文字列の正規表現で当てない。** セッション一覧を出すかの判定は以前
  `/(^|[\\/\s])claude(\s|$)/` だった。表の `sessions` を見る形に置き換えてある。ただし
  **`sessions` は拡張点ではない**（真にすると Claude のセッション一覧が出る）。2 つ目に
  付けるときは、一覧の取得と再開コマンドの組み立てを id で分けるところから
- 設定の `agentCommands` は**利用者が足した行だけ**（既定は空）。表と同じコマンドは
  メニュー側が畳むので、古い設定に残る `claude` 2 行が二重に出ることはない
