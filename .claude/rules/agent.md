...
- **デスクトップ通知は使わない**（#265 で作ってから外した）。知らせるのは**タスクバーの
  点滅**（`window_flash`）と画面内の印の 2 つで、通知は出さない。実機で確かめた順に:
  - WebView2 の Web Notification は `Notification.permission` が `granted` でも**クリックが
    ページへ返らない**（素の `new Notification` に `onclick` を付けて確認）。`tauri-plugin-
    notification` の desktop 実装は `notify_rust` へ投げっぱなしで、クリックを受ける口が
    そもそも無い（`onAction` はモバイル専用）。**トレイのヒント通知（#161）のクリックも、
    同じ理由でずっと効いていない**
  - WinRT のトーストを自前で出す形も試したが（`tauri-winrt-notification` を直接叩き、COM の
    常駐スレッドから登録する版まで書いた）、**押しても `on_activated` が発火しなかった**。
    未パッケージのデスクトップアプリでトーストのアクティベーションを受けるには
    **AppUserModelID を書いたショートカット**の登録が要り、それが無いと借り物の AUMID
    （`notify_rust` と同じく PowerShell のもの）で出すことになる。その状態では**バナーも
    出ず通知センターへ直行**し、送信元名も借りたアプリのものになる
  - ショートカットを登録する道もあるが、開発版でもスタートメニューに実体ができるうえ、
    ジャンプリストの「AUMID を明示設定しない」（`project.md`）ともぶつかる。**知らせる
    目的はタスクバーの点滅で足りる**ので、通知そのものをやめた
  - `lib/notify.ts` は残る（トレイのヒントが使う）。**押させたい知らせをあれに載せないこと**、
    という規約をあのファイルの doc に書いてある
