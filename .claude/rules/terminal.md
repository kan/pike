...
- **キーの重なりは #369 で棚卸しした**。グローバルの表と CodeMirror 層（既定・検索・履歴・Markdown・マクロ）で実際に重なっていたのは `Mod+K`（一覧とリンク挿入）だけ。`Mod+F` / `Mod+H` はどちらも同じ操作。キーを足すときは、この 2 層と `terminalFirst` の 3 か所を見る
