...
- **プロジェクト切替でタブを捨てない（#264）**: `switchProject` は kill せず、`tabStore.setOwnerProject(id)` で見せる相手を差し替えるだけ。ターミナルのプロセスとエージェントのセッションが生きたまま残る（＝切替時の「実行中ですが良いですか」の確認も不要になった）。初めて開くプロジェクトだけ `lastSession` / `pinnedTabs` から復元する
  ...
  - **見えていないチャットは本文を描かない**（`AgentChatTab.vue` の `v-if`）。タブが生き続けるので、描いたままだとトークンが届くたびに全文の markdown 再パースとサニタイズが走る。会話はストア側にあるので描き直しても何も失われない
  - **`tabs`（全部）を直接読んで良いのは 2 つだけ**: id で 1 つ引くとき（タブのコンポーネントは自分の id で引く）と、`TabPane` の**中身**の `v-for`（マウントしたままにするのが目的なので全部要る）。**それ以外は `visibleTabs`**（タブバー・ナビゲーション・`@` モード・ターミナルへの注入・エージェントのエディタ文脈・セッションの書き出し・空表示の判定・既定ターミナルの有無・ファイル変更の反映）
  ...
  - 逃げ道は UI に出す（#264 の判断）: プロジェクト一覧とスイッチャーの「保持中」バッジ、タブバー左の切替チップとその ✕、パネルの電源ボタン、ウィンドウタイトルの `pike - school (sitter / musql)`
    - **解除の実体は `releaseProject` の 1 つ**（パネルとチップが共有）。**タブがあるときだけ確認する**: 復元待ちのものは動いているプロセスが無いので、「実行中のプロセスも終了します」と聞くのは嘘になる。共有する前は、パネルの解放ボタンが復元待ちのプロジェクトに対して何もしなかった
    - ✕ は保持中のチップだけに出す。現在地に出すと「今見ているプロジェクトのタブを全部閉じる」になり、解除とは別の操作になる
  - **一時プロジェクト（#230）はチップ列に出さない**。あれは切り替えると破棄されるので、「戻ってきたらそのままある」という列の約束を満たさない（出るのに戻れない、という食い違いになる）。現在地としても出さない
  - **並びは `heldIds`（順序つきの 1 本）で固定**し、選択でも materialize でも入れ替えない（押すたびに行き先が動くと狙えない）。タブを持った順に足し、手放すまで残す。「まだタブを作っていない」は別のリストではなく `hasTabsFor` の否定で表す（2 本にすると、pending から実体化した瞬間に並びが変わる）。**タイトルだけは現在地が先頭**: タスクバーでは「今どれか」が先に読めるほうが良く、並びを覚えて押す対象でもない
