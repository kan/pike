...
- **Vue SFC のプレビュー（#397）**: 外部コマンド `vue-preview`（kan/vue-preview）が描いた 1 枚の HTML を、HTML のプレビューと同じ子 webview の仮想ファイル（`lib/vuePreview.ts` の `VUE_PREVIEW_ENTRY`）として出す（`components/editor/VuePreview.vue`）。判断の実体は `src-tauri/src/vue_preview.rs` のモジュール doc が正本
  ...
  - **`gh` と同じ「入っていれば使える」形**。検出は `vue_preview_available`（探し方は `ShellConfig::probe_command`＝`has_command` の三値版、Rust での覚え方は `cache::ProbeEntry::found`。どちらも `issues_gh_available` と共有）とストアの `stores/vuePreview.ts`。**描くのは見つかったシェルの .vue だけ**（`isVuePreview`）。検出は .vue のタブを開いたとき（`EditorTab` の watch）で、起動時には聞かない
    - **確かに無いシェルでは、トグルを出して Preview の欄に入れ方の案内を出す**（`isVueInstall` → `components/editor/VuePreviewInstall.vue`）。検出の答えは `found` / `missing` / `unknown` の三値（`types::CommandProbe`）で、**`missing` はシェルが「コマンドが見つからない」と答えたときだけ**（POSIX は 127 だけ。Windows のシェルは `cmd /C` の非 0 で、見つからないときは 1。判定は `probe_command` の doc が正本）。時間切れ（冷えた WSL）で「無い」と言うと、入っている人に入れ直しを勧めて上書きさせる。聞いている途中はどちらにも入れない（トグルが一瞬出て消えるのを避ける）。DOM のプレビューの処理を外す判定は `ownPanePreview`（子 webview と案内の和）
    - **描画に失敗したら `force` で探し直す**（`VuePreview.vue` の `renderOnce`）。Rust は「見つかった」をプロセスの寿命ぶん覚えるので、Pike を動かしたまま vue-preview を消すと、そのままでは「コマンドが見つかりません」の失敗ページが出続ける。失敗の文面はシェルと言語で変わるので見分けず、失敗した回にだけ探し直す
    - 「入れる」は `stores/vuePreview.ts` の `install` がターミナルのタブで 1 行を流し、終わったら `force` で探し直す（ripgrep の `installRipgrep` と同じ形）。1 行は `lib/vuePreview.ts` の `vuePreviewInstallCommand`。GitHub の `releases/latest/download/<名前>` から取り、sha256 を照合してから置く。**置き場は sudo の要らない `~/.local/bin`（Windows のシェルは `%USERPROFILE%\.local\bin`）**で、Pike は起動時にそこを実在しなくても PATH に足す（`platform.md` の PATH の節）。ripgrep と違って `/usr/local/bin` にしなくてよいのは、vue-preview の検出と実行が `has_command` / `run_shell_line`（WSL は `WSL_EXTRA_PATH` を前置する）を通るため
    - **Windows のシェルは cmd・PowerShell・Git Bash のどれでも `powershell -EncodedCommand` の 1 行**（引用を持たないので打ち込む先を選ばない。`-Command "..."` だと、打ち込んだ先の PowerShell が `$` を先に展開する）。**スクリプトの中では cmdlet を使わず .NET を直に呼ぶ**: pwsh から `powershell.exe` を起こすと 7 の `PSModulePath` を継ぎ、`Get-FileHash` が読めずに落ちた（実測）
