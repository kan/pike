...
- **固定タブは左端に据え置き、スクロールしない（#305）**。ブラウザの固定タブと同じで、タブが増えても居場所が変わらない
  - **並べ替えは `visibleTabs` の 1 箇所**（ピン留めを先頭へ寄せる）。**タブバーだけ並べ替えないこと**: `Ctrl+1`〜`9`・タブ移動・溢れたタブの一覧・セッションの書き出しが別の順を見ることになり、「左から n 番目」が画面と食い違う。`tabs` の順（作った順）は動かさない
  - タブバーは `visibleTabs` を切れ目で 2 つに分けるだけ。**タブ 1 枚のマークアップは `TabItem.vue`**（`v-for` が 2 つになったので、写すとバッジやアイコンを片方だけ直す事故が起きる）。見た目の CSS は `TabPane.vue` 側に置いたまま（scoped CSS は子のルート要素まで届く）
  - **グループをまたぐドラッグは受けない**（`sameGroup`）。`moveTab` は `tabs` の順を変えるが、表示は寄せたあとの順なので画面上は何も起きない。印を出さないことで、動かないドロップを試させない
  - アクティブなタブへの追従（`revealActiveTab`）は `.tabs-scroll` の中だけを探す。固定タブはそこに居ないので、選んでもスクロールしない
