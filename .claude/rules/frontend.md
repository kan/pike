...
- **新しいポップアップを足したら `useOverlay` に登録する**（`.popup-surface` を付けるのと
  対の作法。あちらは `project.md` の backdrop の節）。登録は開閉の ref を返す関数を渡す
...
- **隠しているあいだは、その場に理由を出す**（`BrowserTab.vue` の `hiddenNotice`）。
  ...
  **文言は理由で分ける**: Git パネルは開けっぱなしにできるので、閉じれば戻ることを言わないと
  戻し方が分からない
- **スクリーンショットを貼って隠す案は未実装**（#396 で見送った）。WebView2 の
  `CapturePreview` を Rust 側に足せば「止まった絵」を残せるが、撮影が非同期なので開いた
  瞬間の空白は残る。まず隠す側で塞いである
...
- **#375 の `tabAddAction` に 5 つ目の値を足す形ではなく、鍵ごと変えてある。** 既定を
  差し替えても、v0.55.0 で `terminal` が保存済みの人には届かない（`snapshot()` は全
  フィールドを書く）。届かせるには「利用者が自分で選んだ `terminal`」を書き換える移行が
  要り、見分ける手段が無い。**新しい鍵なら `undefined` が既定に落ちる**ので移行が要らない。
  代償は、`editor` などへ変えていた人が 1 度選び直すこと
  - **真偽値を 1 つ足す形（`tabAddDirect` ＋ `tabAddAction`）も採らなかった。** 移行は
    同じく不要になるが、「直接開かないのに開く種別を持っている」という**観測できない
    状態**ができ、永続化の触り所と設定画面の行が 2 つずつになる
...
- **分割を開くときは、見ているタブだけを右へ送る**（`toggleSplit`）。今のプロジェクトの
  タブ（`visibleTabs`）に残っている「右」を先に左へ戻す。以前は右に置いたままのタブを
  拾い直していたので、固定したブラウザのタブが一度でも右に置かれると、分割するたびに
  見ているタブではなくそちらが右に出た（置き場は画面から見えないので、利用者には理由が
  分からない）
...
- **`--accent` / `--danger` の地に載せる文字は `--on-accent`**（#365）。`--text-active` は「強調した文字」で、ライトテーマでは黒になる。青地に黒は読めず、設定画面の選択中のボタンで報告された。直したときは約 50 か所が `--text-active` を地の色に関係なく使っていた。**選択中の行（`.selected` に `--accent` を塗るもの）の子要素も同じ**（アイコン・キーの表記・バッジ）。パスやキーの表記のような副文字は `--on-accent-muted`。`#fff` を直書きしないこと（値を変えたときに取り残される）
...
- **分類と絞り込み（#314）**: 節は 8 つ（全般 / 外観 / ターミナル / エージェント / エディタ / 外部との通信 / 設定の同期 / バージョン情報）で、長い節は小見出しで分ける。器は `components/settings/`（`SettingSection` / `SettingGroup` / `SettingItem` / `SettingToggle` / `HighlightText`）、絞り込みの状態は `composables/useSettingsSearch.ts`
  ...
  - **項目は「名前 → 説明 → 操作」の縦並び 1 つだけ**（#365。VS Code と同じ）。横並び（名前の右端に操作）は、幅の広い画面で名前と操作が離れて対応が読めなかったのでやめた。操作は既定で中身の幅、一覧・並び・入力欄は `wide` で行幅に広げる
