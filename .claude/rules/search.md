...
- **フォルダに絞る（#376）**: ファイルツリーの右クリック「このフォルダ内を検索」が `setScope` で範囲を置いてから `requestOpen` でパネルを開き、検索の起点（rg / grep に渡すパス）をそのフォルダにする。**`scope` は今の `activeRoot` の配下にあるときだけ効く**（`scopeRel`）ので、プロジェクトや worktree を切り替える経路ごとに消す約束が要らない。`clear()` では消さない（検索語を消したときにも呼ばれる）
  ...
  - **帯（「対象: …」）自体がファイルツリーへの入口（#407）**: 絞り込みも解除もあちらの右クリックにしか無いので、ツールチップで教えるだけにしない。**✕ は別のボタンのまま**（`<button>` は入れ子にできない）
    - **`revealFile` を直に呼ばないこと。** `openPanel('files')` のあとに呼ぶと、そのあとマウントされる `FileTreePanel` の `onMounted` が結果を潰す（`ensureInit()` が初回は `initTree` に落ちて展開と選択を捨て、続く `revealActiveFile()` が選択を今のエディタのファイルで上書きする）。依頼は `fileTree` ストアの `requestReveal` に置き、パネル側の `onMounted` が消化する（このパネルの `pendingOpen` と同じ作り）
