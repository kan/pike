...
- **ヒーロー画像の合成には ImageMagick（`magick`）が要る**。マニュアル画像のコピーは要らないので、`magick` が無いと**マニュアル 46 枚だけ更新されてヒーロー 6 枚が古いまま残る**（`sync-hero-images.sh` が `magick: コマンドが見つかりません` で落ちるが、先に走る `sync-manual-images.sh` は成功している）。この PC では Windows 側（`C:\Program Files\ImageMagick-7.0.10-Q16-HDRI`）にあり **WSL の PATH には無い**ので、`npm run e2e:sync` を WSL の bash で回すとここで止まる。**Git Bash から走らせれば通る**（`bash scripts/sync-manual-images.sh && bash scripts/sync-hero-images.sh`）。v0.35.0 の再撮影で実際に踏んだ
...
- StatusBar のエージェント項目は**メーターアイコン＋ 5 時間枠の利用率だけ**（#226）。クリックで開くドロップダウンに Claude と Codex を並べ、内訳はエージェント状態タブへ
