...
- **未 clone プロジェクトを開く（#212）**: 同期（#164）で入ってきたプロジェクトは root がこのマシンに無いことがある。`missingRoots` / `cloneProject` は ProjectPanel 専用だったので、**開く経路の共通入口**として `stores/project.ts` に `ensureRootPresent(id, onCloned)` を置いた（戻り値＝「今 root がある」。false は開いてはいけない＝ URL が無いか clone を開始した。clone 後の再開は `onCloned` 側）。`cloneProject(id, onCloned?)` は onCloned があれば従来の「切り替えますか？」confirm を出さない。呼ぶのは 2 箇所: ProjectSwitcher の `selectProject`（開く前に確認）と、App.vue の `windowProjectId` 分岐（ジャンプリスト / トレイ / 別ウィンドウから開いた後に確認し、clone 完了で `switchProject` し直す）。**プロジェクトを開く経路を足すときはここを通す**
